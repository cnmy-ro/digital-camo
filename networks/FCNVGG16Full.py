...
class FCNVGG16Full(nn.Module):
	"""
	FCN version of VGG16 for multi-class Semantic Segmentation
	- 21 segmentation classes in total
	"""
	def __init__(self, mode='FCN-32s'):
		...
		self.upconv_32s = nn.ConvTranspose2d(in_channels=512, out_channels=21, kernel_size=3, stride=32, padding=0, output_padding=29)
		...
		if self.mode == '3-stream':
			...
			self.scoring_conv_16s = nn.Conv2d(in_channels=1024, out_channels=21, kernel_size=1, stride=1, padding=0)
			...
			self.scoring_conv_8s = nn.Conv2d(in_channels=512, out_channels=21, kernel_size=1, stride=1, padding=0)
...
if __name__ == '__main__':
	...
	"""
	fcn_model = FCNVGG16Full(mode='3-stream')
