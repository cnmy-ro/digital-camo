...
class FCNVGG16FullOutput(nn.Module):
	"""
	Simplified version of FCN-VGG16 with less parameters
	- For multi-class semantic segmenatation of all 21 categories
	"""
	def __init__(self, mode='FCN-32s'):
		...
		if self.mode == '3-stream':
			...
			# Modules for FCN-8s
			self.upconv15_2x_2x = nn.ConvTranspose2d(in_channels=64, out_channels=32, kernel_size=3, stride=2, padding=1, output_padding=1)
	...
	def forward(self, X):
		...
		if self.mode == '3-stream':
			...
			conv15_2x_out = self.relu( self.upconv15_2x( 
			               			 ...
			               			 )   # 2x upsampling of Conv15 output - (64,16,20)
			...
			conv15_4x_out = self.relu( self.upconv15_2x_2x( 
									 ...
									 ) # 4x upsampling of Conv15 output - (32,32,40)
...
if __name__ == '__main__':
	...
	"""
	model = FCNVGG16FullOutput(mode='3-stream')
