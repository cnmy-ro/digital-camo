...
def main():
    ...
    logger.info(f"Total CUDA devices: {n_devices}")
    logger.info(f"Current device: {current_device}")
    logger.info(f"GPU 0 name: {gpu0_name}")
    logger.info("\n")
    ...
    logger.info(f"Trainable parameters: {param_list}")
    ...
    for e in range(1, TRAINING_CONFIG['epochs']+1):
        ...
        logger.info(f"Training IoU: {train_iou_score}")
        ...
        logger.info(f"Validation IoU: {val_iou_score}")
